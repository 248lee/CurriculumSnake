...
def main():
    directory = "./coaches"
    # Get the list of filenames in the specified directory
    coaches_name_list = [filename for filename in os.listdir(directory) if os.path.isfile(os.path.join(directory, filename))]
    for cfn in  coaches_name_list:
        coach_name = cfn[:len(cfn) - 4]
        ExperimentName = "mc_value_evaluation_" + coach_name  + "_in_BOSS"
        ...
        model.set_old_policy_model("coaches/" + coach_name)
